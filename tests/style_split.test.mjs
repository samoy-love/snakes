...
test('grid-свойства не назначаются элементам, переведённым в flex', () => {
  ...
  // .topHudRight — та же история этажом ниже: правило под телефон продолжало
  // задавать ему колонки сетки уже после перевода строки в flex.
  assert.equal(
    /\.topHudRight\s*\{[^}]*grid-template-columns/.test(css),
    false,
    '.topHudRight выложен flex-ом — grid-template-columns на нём не действует'
  );
});

/* --- Подсказки управления живут ВНЕ #hud ------------------------------------
   #help и #helpTouch вынесены из #hud намеренно: у #hud есть filter и
   backdrop-filter, они создают containing block, и position: fixed внутри
   ломается (см. комментарий в index.html). Любой селектор вида `#hud … #help`
   после этого не совпадает ни с чем, но выглядит рабочим — так в файлах
   накопились четыре мёртвых правила и встречные !important, спорившие друг
   с другом о видимости подсказки, которой ни одно из них не управляло. */
test('подсказки управления не адресуются как потомки #hud', () => {
  const css = stripComments(allCss);
  assert.ok(
    !/#hud[^,{}]*#help/.test(css),
    '#help/#helpTouch лежат вне #hud (index.html) — селектор `#hud … #help` мёртв'
  );
});

/* --- Старая палитра не возвращается ----------------------------------------
   Редизайн (волна 9) перевёл продукт на изумруд + циан, но покрасил только
   статику: фоны, рамки, кнопки. СОСТОЯНИЯ — выбранная и надетая карточка,
   активная вкладка и фильтр, статус покупки, свои строки в таблицах, бейджи
   и ползунки — остались на прежних литералах rgba(255,45,85) и
   rgba(96,165,250). Магазин выглядел наполовину перекрашенным: карточка
   изумрудная, а выделенная — синяя из старой темы.

   Литералы заменены на токены. Тест держит границу: цвет берётся из
   переменной, а не вшивается числом.

   Исключение ровно одно — сам --accent. Он ОСТАЁТСЯ красным: по правилу
   редизайна красный закреплён за опасностью и смертью. */
test('в стилях нет литералов старой сине-красной палитры', () => {
  const OLD_BLUE = /rgba?\(\s*96\s*,\s*165\s*,\s*250/;
  const OLD_RED = /rgba?\(\s*255\s*,\s*45\s*,\s*85/;

  for (const f of cssFiles) {
    const src = stripComments(readCss(f));
    assert.equal(OLD_BLUE.test(src), false, `${f}: старый синий 96,165,250 — используйте var(--brand2)`);

    // Красный разрешён ровно в одном месте: объявлении токена --accent.
    const redLines = src
      .split('\n')
      .filter((l) => OLD_RED.test(l))
      .filter((l) => !/--accent:/.test(l));
    assert.deepEqual(
      redLines.map((l) => l.trim()),
      [],
      `${f}: старый красный вне токена --accent — используйте var(--accent)`
    );
  }
});
