...
test('connect() поверх незавершённого хендшейка — no-op, а не второй сокет', () => {
  // Ловит: клик «Играть» дважды до открытия сокета создавал второй CONNECTING
  // поверх первого — оба делили общий модульный state, проигравшую пару
  // сервер рвал, и пользователь видел «переподключение» ещё до первого
  // онлайна. Гвард — только на CONNECTING: явный connect() поверх уже
  // ОТКРЫТОГО сокета по-прежнему обязан форсировать новый (см. тесты выше).
  const env = setup();
  try {
    env.net.connect();
    assert.equal(env.ws.sockets.length, 1);

    env.net.connect();
    assert.equal(env.ws.sockets.length, 1, 'второй connect() до открытия не должен создавать сокет');

    env.ws.last().fireOpen();
    env.net.connect();
    assert.equal(env.ws.sockets.length, 2, 'connect() поверх ОТКРЫТОГО сокета обязан форсировать новый');
  } finally {
    env.restore();
  }
});
