...
**Рассинхрон протокола ловится тестами — потому что он ломал игру молча.**
Байтовая раскладка трижды за проект расходилась между сервером и клиентом:
страница открывалась, просто замолкали киллфид и баланс. Теперь эталонные
буферы снимаются с боевых Go-сериализаторов, и оба конца — Go-тесты и
node-тесты — сверяются с этим эталоном на CI.
...
меньше, чем больше пришло людей. У них три тира сложности и четыре архетипа
поведения (Фермер, Агрессор, Трус, Территориальный), а перед броском на
жертву бот несколько тиков идёт по прямой — видимый замах, на который человек
