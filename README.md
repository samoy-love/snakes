...
| `BIND_ADDR` | `127.0.0.1` | `server.go` | Интерфейс привязки. Дефолт — только loopback: в проде сервер стоит за СИСТЕМНЫМ nginx и снаружи напрямую доступен быть не должен. `docker-compose.yml` выставляет `0.0.0.0` сам — там nginx отдельный контейнер и ходит по compose-сети |
