	...
	for {
		...
		if client.ip != "" {
			...
			case "input":
				rate, burst = 30, 60
			case "rttPing":
				// G-lag: burst 4 давал закрыть себе рот самому — pingTimer в
				// client_net.js стартует на КАЖДОМ ws.onopen, и пара
				// реконнектов подряд (реальная сетевая нестабильность, не
				// злоупотребление) исчерпывала ведро раньше, чем клиент успевал
				// закрепиться. Устойчивый темп (rate) не трогаем — только запас
				// на всплеск при переподключении.
				rate, burst = 2, 8
			case "setName":
				// G-lag: setName шлётся не только по явному действию игрока, но
				// и автоматически на каждом ws.onopen (client_net.js) — это
				// единственный способ напомнить серверу имя после реконнекта,
				// сервер иначе не восстанавливает его сам. При нескольких
				// реконнектах подряд burst=2 гарантированно рвал соединение
				// повторно ДО того, как оно успевало устояться — жалоба на
				// «висит переподключение и игра не начинается» с логами
				// ws_rate_limited type="setName" это подтвердила. Устойчивый
				// темп (1/сек) всё ещё блокирует спам явного переименования.
				rate, burst = 1, 6
			case "clientTiming":
				rate, burst = 1, 2
			case "leave":
