...
func TestWSOriginAllowlistIsTheOnlyArbiter(t *testing.T) {
	...
	t.Cleanup(httpx.SetWSOriginPolicy([]string{"http://example.test:18080"}, true))
...
func TestEquipDoesNotMaterializeProfile(t *testing.T) {
	...
	profiles.Mu.Unlock()
	if pr == nil {
		t.Fatal("profiles.ForKeyEquipLocked вернул nil")
	}
	if stored {
		t.Fatal("несуществующий профиль отдан как сохранённый")
	}
	if n := profiles.Len(); n != 0 {
	...
	for _, cat := range []string{"capturefx", "head", "seg", "nameplate", "frame", "terr", "death"} {
		...
		if profiles.EquippedLocked(pr, cat) != 0 {
			profiles.Mu.Unlock()
	...
	real := profiles.ForKeyCreate(pid)
	profiles.Mu.Lock()
	got, stored2 := profiles.ForKeyEquipLocked(pid)
	profiles.Mu.Unlock()
	if got != real || !stored2 {
		t.Fatal("сохранённый профиль не найден путём экипировки")
	}
	profiles.Mu.Lock()
	empty, _ := profiles.ForKeyEquipLocked("")
	profiles.Mu.Unlock()
	if empty != nil {
