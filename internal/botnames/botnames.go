// Package botnames — генератор ников для ботов: пулы слов (русские и
// английские) и подбор уникального имени внутри комнаты.
//
// Пакет отдельный, потому что генератор ничего не знает об игре: ему нужны
// только источник случайности и множество уже занятых имён. Держать три сотни
// строк словарей рядом с логикой захвата территории — значит каждый раз
// проходить мимо них, читая игровой код.
package botnames
...
// Pools groups one language's word lists so the generator can be run
...
func StartKey(nm string) string {
	s := nm
	for {
...
func PickUnique(rng *rand.Rand, pools Pools, used map[string]struct{}, usedStarts map[string]struct{}, fallbackN int) string {
	if used == nil {
