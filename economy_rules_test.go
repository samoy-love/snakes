...
func TestRubberBandIsClampedBothWays(t *testing.T) {
	...
	// Лидер получает ровно базу. Комментарий в awardPoints обещает «-10%,
	// когда далеко впереди», но `best` считается по ВСЕМ игрокам, включая
	// самого получателя, поэтому d = best-me никогда не отрицателен и ветка
	// штрафа недостижима. Тест фиксирует фактический контракт: множитель
	// лидера равен 1.0 и никогда не меньше — если кто-то починит `best`
	// (например, начнёт исключать самого игрока), выплаты лидеру просядут
	// молча, и это должно быть осознанным изменением.
	r = newRoom(60000, 60000)
	before := r.points[1]
	r.awardPoints(1, 100, PointsCapture)
	if got := r.points[1] - before; got != 100 {
		t.Fatalf("лидер получил %d очков за базу 100, ожидалось 100", got)
	}
	// И множитель не опускается ниже 1 ни на какой позиции.
	for _, best := range []uint16{20, 100, 1000, 60000} {
		...
		if got := rr.points[1] - best; got < 100 {
			t.Fatalf("при best=%d начислено %d очков за базу 100 (<100)", best, got)
		}
	}
