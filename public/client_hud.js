...
export function renderTeamHudImpl() {
  ...
  /* Итерация 6: только эта, заполненная, ветка не звала syncRightEmptyStates()
     — ранняя (session.started===false) уже дёргала её и показывала плашку
     «Пока пусто», а тут её никто не прятал обратно. На desktop-2560x1440
     панель просторнее max-height, поэтому неубранная плашка попадала в
     видимую область прямо под таблицей с реальными данными (см.
     match-scene-killfeed-busy.png). На узких вьюпортах тот же кусок разметки
     оставался, просто обрезался высотой панели — баг был замаскирован, а не
     отсутствовал. */
  try {
    syncRightEmptyStates();
  } catch {}
