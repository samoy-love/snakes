...
export function paintBursts(ctx, cameraFrame, nowFrame, deps) {
  ...
  for (let i = bursts.length - 1; i >= 0; i--) {
    ...
    if (isScore) {
      ...
      // Стартовое смещение вверх (0.6 клетки) — не даёт «+N» появиться ровно
      // поверх подписи длины змейки под головой в момент спавна (age≈0,
      // easeOutCubic(sp)≈0): раньше подъём начинался только за счёт eased-
      // члена, который у самого начала анимации почти нулевой, и оба текста
      // читались наложенными друг на друга (docs/reviews/iter-3.md). Итоговая
      // максимальная высота подъёма (0.6 + 0.9 = 1.5 клетки) осталась близка
      // к прежней (1.2 клетки), только стартует выше, а не с нуля.
      // Фиксированный сдвиг в клетках был откалиброван по десктопному
      // cell (~25-51px) — на iphone-390x844 cell вдвое меньше (~15px:
      // вписывание VIEW_CELLS_X=40 в 390px давало 9px, потолок видимых
      // клеток поднял до 15), а кегль текста size растёт от v и не
      // зависит от cell вовсе. При
      // v=64 (size≈34px) подъём в клетках даёт на мобильном лишь ~14px
      // против ~50px+ на десктопе — почти вчетверо меньше при том же
      // размере цифр, поэтому «+64» вновь касалось ripple-кольца и
      // подписи длины именно на мобильном вьюпорте (docs/reviews/iter-5.md).
      // Берём максимум клеточного и завязанного на size подъёма — на
      // десктопе (большой cell) работает прежняя клеточная формула, на
      // мобильном (крошечный cell) подъём держится на размере самого
      // текста и не проседает вместе с cell.
      const liftBase = Math.max(cell * 0.6, size * 0.55);
      const liftEase = Math.max(cell * 0.9, size * 0.8) * easeOutCubic(sp);
      const sx = offsetX + (x + 0.5) * cell;
      const sy = offsetY + (y + 0.5) * cell - liftBase - liftEase;
