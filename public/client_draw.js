...
export function paintBursts(ctx, cameraFrame, nowFrame, deps) {
  ...
  for (let i = bursts.length - 1; i >= 0; i--) {
    ...
    if (isScore) {
      ...
      // cell (~25-51px) — на iphone-390x844 cell схлопывается до ~9px
      // (MIN_CELL=6, cellSizeFor вписывает VIEW_CELLS_X=40 в 390px), а
      // кегль текста size растёт от v и не зависит от cell вовсе. При
