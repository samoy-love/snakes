...
func TestCapturePaysOncePerRealGain(t *testing.T) {
	...
	// ВНИМАНИЕ. Оплачивается НЕ огороженная площадь, а длина следа: capture()
	// снимает ownedBefore уже ПОСЛЕ заливки внутренней области, поэтому в
	// delta попадают только клетки самого следа. Это соответствует
	// комментарию G1 в capture() («единица — след, нарисованный снаружи»), но
	// прямо противоречит доке самой capturePoints («11 очков за 100 клеток,
	// 31 за 400» — там delta трактуется как площадь). Тест фиксирует
	// фактический и, судя по G1, задуманный контракт: платит след.
	delta := trailLen
